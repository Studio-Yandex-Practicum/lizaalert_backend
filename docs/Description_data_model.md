...
Необходимо дополнить встроенную модель полем "Номер телефона" (phone_number) с использованием его в качестве уникального идентификатора пользователя. Дополнить 
втроенную модель полем "Полное имя" (full_name). Отключить (задать равными None) поля username, first_name и last_name.
...
- **location*** - географический регион, тип CharField, выбор из заданного списка субъектов РФ
