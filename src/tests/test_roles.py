...
class TestRoleAPI:
    ...
    def test_create_user_roles(self, client, user, role_main_admin, role_teacher):
        ...
        try:
            ...
            pass
        assert (
            UserRole.objects.filter(user=user).count() == 2
    ...
    def test_delete_user_roles(self, client, user, user_admin_teacher_role):
        ...
        assert not UserRole.objects.filter(
            user=user, id=role_id
        ).exists(), "Проверьте, что метод DELETE удаляет нужную роль у пользователя."
    ...
    def test_admin_user_roles(self, client, user, role_main_admin, role_teacher):
        ...
        assert not User.objects.get(
            id=user.id
        ).is_superuser, "Пользователь, не имеющий роль main admin, не должен иметь прав суперпользователя."

        client.post(f"/api/v1/users/{user.id}/roles/", data=user_role_admin_data)
        assert User.objects.get(
            id=user.id
        ).is_superuser, "Пользователь, имеющий роль main admin, должен иметь права суперпользователя."
