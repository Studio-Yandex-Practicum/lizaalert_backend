...
def assign_achievements(user, course_id):
    ...
    for badge in badges_for_course:
        if not user_received_badge_for_course or not user_received_badge_for_course.filter(badge=badge).exists():
            VolunteerBadge.objects.create(volunteer=user.volunteer, badge=badge, course_id=course_id)
...
def increment_completed_courses_count(user):
    ...
    volunteer_completion, created = VolunteerCourseCompletion.objects.get_or_create(volunteer=volunteer)
