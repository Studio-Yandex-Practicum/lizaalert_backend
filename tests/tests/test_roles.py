...
class TestRoleAPI:
    ...
    def test_user_roles_url(self, client, user):
        response = client.get(f"/api/v1/users/{user.id}/role/")

        code = 200
        assert response.status_code == code, ("Эндпоинт /api/v1/users/{user.id}/role/ недоступен.")
    ...
    def test_create_user_roles(self, client, user, role_main_admin, role_teacher):
        ...
        client.post(f"/api/v1/users/{user.id}/role/", data=user_role_data_1)
        ...
        client.post(f"/api/v1/users/{user.id}/role/", data=user_role_admin_data)
        assert user_roles.count() == 2, ("У пользователя может быть несколько ролей.")

        try:
            client.post(f"/api/v1/users/{user.id}/role/", data=user_role_incorrect_data)
        except Exception:
            pass
    ...
    def test_delete_user_roles(self, client, user, user_admin_teacher_role):
        ...
        client.delete(f"/api/v1/users/{user.id}/role/{role_id}/")
